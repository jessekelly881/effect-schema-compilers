...
const generatesValidEq = <I, A>(schema: S.Schema<I, A>) => {
  ...
  const transitivity = fc.property(
    ...
    arb,
    arb,
    (a, b, c) => (eq(a, b) && eq(b, c)) === (eq(a, b) && eq(a, c))
  );
...
describe("equivalence", () => {
  it("ast", () => {
    const ast = pipe(S.NumberFromString, _.equivalence(Eq.strict())).ast
      .annotations;
    expect(ast).toEqual({
  ...
  it("tuple/ e + r", () => {
    ...
    expect(
      eq(["", { a: true }, { a: true }], ["", { a: true }, { a: false }])
    ).toEqual(false);
  });
  ...
  it("tuple/ e + r + e", () => {
    const schema = pipe(
      S.tuple(S.string),
      S.rest(S.struct({ a: S.boolean })),
      S.element(S.struct({ b: S.string }))
    );
    const eq = _.to(schema)();

    generatesValidEq(schema);
    expect(
      eq(["", { a: false }, { b: "" }], ["", { a: true }, { b: "" }])
    ).toEqual(false);
    expect(
      eq(["", { a: true }, { b: "" }], [" ", { a: true }, { b: "" }])
    ).toEqual(false);
    expect(
      eq(
        ["", { a: true }, { a: true }, { b: "" }],
  ...
  it("lazy/ ", () => {
    ...
    expect(
      eq({ name: "a", subcategories: [] }, { name: "a", subcategories: [] })
    ).toEqual(true);
    expect(
      eq(
