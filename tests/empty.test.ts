...
const expectEmptyValues = <A, I>(schema: S.Schema<I, A>, from: I, to: A) => {
  ...
  expect(emptyTo).toEqual(to), expect(emptyFrom).toEqual(from);
};
...
describe("empty", () => {
  ...
  it("enum", () =>
    expectEmptyValues(S.enums(Fruits), Fruits.Apple, Fruits.Apple));
  ...
  it("nonEmptyArray", () =>
    expectEmptyValues(S.nonEmptyArray(S.string), [""], [""]));
  it("object", () => expectEmptyValues(S.object, {}, {}));

  it("templateLiteral. a", () =>
    expectEmptyValues(S.templateLiteral(S.literal("a")), "a", "a"));
  it("templateLiteral. ${string}", () =>
    expectEmptyValues(S.templateLiteral(S.string), "", ""));
  it("templateLiteral. a${string}", () =>
    expectEmptyValues(S.templateLiteral(S.literal("a"), S.string), "a", "a"));
  ...
  it("number/ greaterThan", () =>
    expectEmptyValues(pipe(S.number, S.greaterThan(4)), 0, 4));
  it("number/ greaterThanOrEqualTo", () =>
    expectEmptyValues(pipe(S.number, S.greaterThanOrEqualTo(4)), 0, 4));
  it("number/ int, greaterThan", () =>
    expectEmptyValues(pipe(S.number, S.int(), S.greaterThan(4)), 0, 5));
  it("bigint/ greaterThan", () =>
    expectEmptyValues(pipe(S.bigint, S.greaterThanBigint(4n)), "", 5n));
  it("bigint/ greaterThanOrEqualTo", () =>
    expectEmptyValues(
      pipe(S.bigint, S.greaterThanOrEqualToBigint(4n)),
      "",
      4n
    ));
  it("string/ minLength", () =>
    expectEmptyValues(pipe(S.string, S.minLength(2)), "", "  "));
  it("array/ minItems", () =>
    expectEmptyValues(pipe(S.array(S.string), S.minItems(2)), [], ["", ""]));
